...
A second "kind" of task alongside project tasks, for repetitive operational work (revisar
pendientes, seguimiento de clientes, reporte semanal, respaldos, etc.) that happens every week on
a given weekday — no more manually re-creating the same task every Monday. From the "Tareas" page:

- **"+ Nueva tarea recurrente"** — name, description, responsable, día de la semana, hora
  opcional, horas estimadas, prioridad, categoría opcional, cliente/proyecto opcionales (sin
  proyecto por defecto), fecha de inicio, fecha de finalización opcional.
- **"Tareas recurrentes" tab** — manage every recurring task in your org: edit, pausar/reactivar,
  eliminar (con la opción de solo detener ocurrencias futuras conservando el historial, o eliminar
  todo), historial de ocurrencias, y métricas simples de cumplimiento de la semana actual.
- **"Calendario semanal" tab** — a Monday–Sunday grid showing that week's project tasks (by
  `due_date`) and recurring occurrences side by side, marked with a discreet "↻ Recurrente" tag,
  with previous/next-week navigation.
- **"Hoy"** — today's recurring occurrences assigned to you appear in their own section, separate
  from project tasks, with quick status change (Pendiente/En progreso/Completada/Omitida).
- **Hours/capacity** — recurring tasks' `estimated_hours` count toward the Dashboard's "Horas
  pendientes" and the Team page's workload, with a small "⚠️ Sobreasignado" indicator when a
  user's hours for today exceed their `daily_available_hours`.

No occurrences are pre-generated by a background job (there's no server to run one) — see
`DATABASE.md`'s "duplicate-avoidance strategy" for how this stays duplicate-free across week
navigation. See `DATABASE.md` for the schema and `SECURITY.md` for the RLS policies and live
cross-org/cross-role verification.

## Finanzas module

A per-organization finance module (`/app/finanzas`) so each company can answer, in under 30
seconds: how much came in, went out, was spent, is owed in taxes, was earned, is pending to
collect, and is pending to pay. Tabs: **Dashboard** (period selector + KPIs: Ingresos, Egresos,
Gastos, Impuestos, Utilidad neta, Flujo de efectivo), **Ingresos** and **Egresos** (CRUD with
category/estado/fecha filters, quick status toggle, inline "+ nueva categoria"), **Impuestos**
(provisiones fiscales, always labeled "Estimacion financiera" — never an official SAT filing),
**Cuentas por cobrar** / **Cuentas por pagar** (pending ingresos/egresos with vencido/por-vencer
indicators), and **Analisis** (ingresos vs. egresos and utilidad by month, gastos por categoria,
margen neto, burn rate, simple proyeccion de cierre de mes).

Deliberately not double-entry bookkeeping, bank reconciliation, CFDI, or SAT integration — see
`DATABASE.md` for the schema and `SECURITY.md` for the `finance_access` permission model (a
...
/app/projects
/app/projects/:id
/app/tasks              (tabs: Lista / Tareas recurrentes / Calendario semanal)
/app/clients
/app/clients/:id
/app/team
