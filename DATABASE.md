...
- `010_recurring_tasks.sql` — weekly recurring tasks: new `recurring_tasks` (template/config) and
...
## Weekly recurring tasks (`010_recurring_tasks.sql`)

A second "kind" of task alongside project tasks: an operational task that repeats every week on a
given weekday (revisar pendientes, seguimiento de clientes, reporte semanal, respaldos, etc.),
without a project attached by default. Two tables:

### `recurring_tasks`
The template/config — one row per recurrence, a new *top-level* org-scoped entity (unlike `tasks`,
it carries its own `organization_id` column directly, since it isn't reached via `project_id`:
`project_id` here is optional, matching "por defecto SIN proyecto" in the spec).
...
| weekday          | text    | `'MON'..'SUN'`, checked — text code rather than a raw 0-6 int, matching this codebase's existing style of checked text enums (`tasks.status`, `tasks.priority`, `projects.status`) |
| time_of_day      | time    | optional |
| estimated_hours  | numeric(5,2) | nullable, same constraint style as `tasks.estimated_hours` |
| priority         | text    | `'LOW'\|'MEDIUM'\|'HIGH'`, checked, default `'MEDIUM'` |
| category         | text    | optional |
| client_id        | uuid FK → clients.id, `on delete set null` | optional |
| project_id       | uuid FK → projects.id, `on delete set null` | optional |
| start_date       | date, not null | defaults to `current_date` |
| end_date         | date    | optional; `check (end_date is null or end_date >= start_date)` |
| active           | boolean, not null, default `true` | pausing sets this `false` |
...
Materialized status **overrides only** — NOT one row per week forever. There is no server/cron in
this app (same $0-infra constraint as `005_agency_features.sql`'s "due soon notifications"), so
future occurrences are never pre-generated by a background job. Instead:

- Which dates a given `recurring_tasks` row *would* occur on is pure calendar-date arithmetic done
  client-side (`src/lib/recurringDates.ts`): matching `weekday`, inside `[start_date, end_date or
  infinity]`, `active = true`. No DB row is needed for a still-pending occurrence — `PENDING` is
  the implicit default for any date that matches but has no row here.
- A row is only created once someone changes an occurrence's status away from `PENDING`
  (`IN_PROGRESS`/`DONE`/`SKIPPED`), via an **upsert** on `(recurring_task_id, occurrence_date)` —
  `insert ... on conflict (recurring_task_id, occurrence_date) do update`. The unique constraint
  below is what makes this safe to call repeatedly (e.g. revisiting the same week twice in the UI)
  without ever creating a duplicate row — see `SECURITY.md` for the live-tested confirmation of
  this.
...
| status             | text    | `'PENDING'\|'IN_PROGRESS'\|'DONE'\|'SKIPPED'`, checked, default `'PENDING'` |
| completed_at       | timestamptz | nullable |
| completed_by       | uuid FK → profiles.id, `on delete set null` | nullable |
| created_at / updated_at | timestamptz | `updated_at` auto-updated via `set_updated_at()` |

Unique constraint: `(recurring_task_id, occurrence_date)`.

**Week boundary**: Monday–Sunday (ISO week), matching the existing "disponibilidad de lunes a
sabado" convention on `profiles.daily_available_hours` / Team.tsx, and Team.tsx's own
`startOfWeek()` helper (also Monday-based). All date math is done on plain `'YYYY-MM-DD'` strings
(never `new Date(dateString)`, which parses as UTC midnight and can shift a day depending on local
timezone) — see `src/lib/recurringDates.ts`.

**Hours/capacity**: `recurring_tasks.estimated_hours` participates in the same aggregates already
built for `tasks.estimated_hours`/`profiles.daily_available_hours` in
`006_task_estimation_capacity.sql` — Dashboard's "Horas pendientes" now also sums this week's
still-open recurring occurrences, and the Team page's new "Carga de hoy" column sums today's
recurring occurrences alongside today's due project tasks, with a small "⚠️ Sobreasignado" badge
